...
| 3 | Hook deploy: create fn + grants (psql, DB-first) + platform enable (Auth → Hooks) | api-fable deploys · connect-fable verifies | **DONE 2026-07-02** — SQL applied (dexter-api 18ae916c), Branch enabled the hook 20:39Z, real-mint decoded payload verified byte-exact vs CONTRACT §4 (api-fable 2043Z mail); function + DEFINER + grants independently confirmed in pg_proc by this seat. **P0a is closed end-to-end.** |
...
| 6 | Publish 0.18.0 + Rule #7 consumer sweep | connect-fable | **Published 2026-07-02** (npm, tag latest). Sweep: board server-side migrated to 0.18.0; dexter-fe DONE (fe seat's coordinated bump 3c488bb: vault ^0.30 + connect ^0.18; video-route auth migrated to offline verify d86e770, live-verified). Still open: the board login page's esm.sh `@0.16.0` client import; dexter-api's own call-home verify sites (utils/supabase.ts, supabaseAdmin.ts, moltbotLink.ts) — commissioned to api-fable. |
